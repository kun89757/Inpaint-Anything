...
def demo(model):
    ...
    def get_inpainted_img(img, mask0, mask1, mask2, text_prompt):
        print(text_prompt)
        ...
        for mask in [mask0, mask1, mask2]:
            ...
            img_replaced_normalized = normalize_img(img_replaced)
            out.append(img_replaced_normalized)
        return out

    def get_single_inpainted_img(img, mask, text_prompt):
        device = "cuda" if torch.cuda.is_available() else "cpu"
        if len(mask.shape) == 3:
            mask = mask[:, :, 0]
        img_replaced = replace_img_with_sd(
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row().style(mobile_collapse=False, equal_height=True):
            ...
            with gr.Column(variant="panel"):
                ...
                text_prompt = gr.Textbox(label="Prompt Text", lines=1, default="", placeholder=None)
                sam_mask = gr.Button("Predict Mask", variant="primary").style(full_width=True, size="sm")
                stable_diffusion = gr.Button("Inpaint Image", variant="primary").style(full_width=True, size="sm")
                clear_button_image = gr.Button(value="Reset", label="Reset", variant="secondary").style(full_width=True,
                                                                                                        size="sm")
            ...
            with gr.Column():
                ...
                with gr.Row():
                    with gr.Column():
                        ...
                        inpaint_img_0 = gr.Button("Inpaint Image", variant="primary").style(full_width=True, size="sm")
                    with gr.Column():
                        img_with_mask_1 = gr.Plot(label="Image with Segmentation Mask 1")
                        inpaint_img_1 = gr.Button("Inpaint Image", variant="primary").style(full_width=True, size="sm")
                    with gr.Column():
                        img_with_mask_2 = gr.Plot(label="Image with Segmentation Mask 2")
                        inpaint_img_2 = gr.Button("Inpaint Image", variant="primary").style(full_width=True, size="sm")

        with gr.Row(variant="panel"):
            with gr.Column():
                with gr.Row():
                    gr.Markdown("## Image Replaced with Mask")
                with gr.Row():
                    img_rm_with_mask_0 = gr.outputs.Image(
                        type="numpy", label="Image Replaced with Segmentation Mask 0").style(height="200px")
                    img_rm_with_mask_1 = gr.outputs.Image(
                        type="numpy", label="Image Replaced with Segmentation Mask 1").style(height="200px")
                    img_rm_with_mask_2 = gr.outputs.Image(
                        type="numpy", label="Image Replaced with Segmentation Mask 2").style(height="200px")
        ...
        inpaint_img_0.click(
            get_single_inpainted_img,
            [img, mask_0, text_prompt],
            img_rm_with_mask_0
        )

        inpaint_img_1.click(
            get_single_inpainted_img,
            [img, mask_1, text_prompt],
            img_rm_with_mask_1
        )

        inpaint_img_2.click(
            get_single_inpainted_img,
            [img, mask_2, text_prompt],
            img_rm_with_mask_2
        )
        ...
        clear_button_image.click(
            reset,
            [img, features, img_pointed, w, h, mask_0, mask_1, mask_2, img_with_mask_0, img_with_mask_1,
             img_with_mask_2,
             img_rm_with_mask_0, img_rm_with_mask_1, img_rm_with_mask_2],
            [img, features, img_pointed, w, h, mask_0, mask_1, mask_2, img_with_mask_0, img_with_mask_1,
             img_with_mask_2,
